 ...
 * 修改著作权Query
 * @Author: xiebaobiao
 * @Date: 2019/2/15 0015 14:18
 */

class ModifyCopyrightQuery {
    @NotNull
    var id: Long? = null
    /**
     * 著作权名称
    ...
    override fun toString(): String {
        return "ModifyCopyrightQuery(id=$id, name=$name, number=$number, type=$type, applicant=$applicant, drawer=$drawer, drawDepartment=$drawDepartment, note=$note, category=$category, status=$status, changed=$changed, registerNumber=$registerNumber, finishedTime=$finishedTime, registerTime=$registerTime, paymentInvoice=$paymentInvoice, author=$author, petitionMaterials=$petitionMaterials, registerNumberMaterials=$registerNumberMaterials, paymentInvoiceMaterial=$paymentInvoiceMaterial)"
    }
